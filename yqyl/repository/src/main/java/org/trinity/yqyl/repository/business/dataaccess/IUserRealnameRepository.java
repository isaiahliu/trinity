        ...
        final Specification<UserRealname> specification = (root, query, cb) -> {
            ...
            if (searchingDto.getStatus().isEmpty()) {
                if (!searchingDto.isSearchAllStatus()) {
                    predicates.add(cb.equal(root.get(UserRealname_.status), RealnameStatus.VERIFIED));
                }
            } else {
                final In<RealnameStatus> in = cb.in(root.get(UserRealname_.status));
                searchingDto.getStatus().stream().map(item -> LookupParser.parse(RealnameStatus.class, item))
