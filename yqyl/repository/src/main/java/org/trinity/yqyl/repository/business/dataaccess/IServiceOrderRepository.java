...
public interface IServiceOrderRepository extends IJpaRepository<ServiceOrder, ServiceOrderSearchingDto> {
    @Query("select count(0) from ServiceOrder where serviceReceiverClient.user=:user and status in (:status)")
    ...
    default Page<ServiceOrder> query(final ServiceOrderSearchingDto searchingDto, final Pageable pagable) {
        final Specification<ServiceOrder> specification = (root, query, cb) -> {
            ...
            default:
                predicates
                        .add(cb.equal(root.join(ServiceOrder_.serviceReceiverClient).join(ServiceReceiverClient_.user).get(User_.username),
                                searchingDto.getCurrentUsername()));
                break;
            }

            if (!StringUtils.isEmpty(searchingDto.getReceiverUserName())) {
                predicates.add(cb.like(root.join(ServiceOrder_.serviceReceiverClient).join(ServiceReceiverClient_.user).get(User_.username),
