layoutApp.controller('contentController', function($scope, $http, $window, errorHandler, $filter, serviceOrderId, mode) {
	...
	$scope.prepareModify = function() {
		...
		$scope.serviceOrder.newServiceDate = new Date($scope.serviceOrder.serviceDate);
	...
	$scope.applyModify = function() {
		...
		if ($scope.serviceOrder.newServiceDate.getTime() != $scope.serviceOrder.serviceDate.getTime()) {
