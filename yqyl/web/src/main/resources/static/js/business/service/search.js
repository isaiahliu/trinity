layoutApp
		.controller(
				...
				function($scope, $http, $window) {
					$http({
						method : "GET",
						url : "/ajax/service/category/parents"
					...
					$scope.selectCategory = function(category) {
						...
						if (category.children == undefined) {
							$http(
									{
										method : "GET",
										url : "/ajax/service/category/children/"
												+ category.id
									}).success(function(response) {
						...
						if ($scope.selectedCategory != undefined) {
							...
							for (var index = 0; index < $scope.selectedCategory.children.length; index++) {
								if ($scope.selectedCategory.children[index].checked) {
									...
									$scope.searchingSubCategories += "categoryChildren="
											+ $scope.selectedCategory.children[index].id;
								}
							}
						}
