		...
		.controller(
				...
				function($scope, $http, $window) {
					...
					$scope.populateUnprocessedOrders = function() {
						...
						ajaxUrl += "?pageIndex="
								+ ($scope.unprocessedOrderPaging.pageIndex - 1);
						ajaxUrl += "&pageSize="
								+ $scope.unprocessedOrderPaging.pageSize;
					...
					$scope.populateProcessedOrders = function() {
						...
						ajaxUrl += "?pageIndex="
								+ ($scope.processedOrderPaging.pageIndex - 1);
						ajaxUrl += "&pageSize="
								+ $scope.processedOrderPaging.pageSize;
						ajaxUrl += "&sortedBy=id_desc";
					...
					$scope.populateProcessingOrders = function() {
						...
						ajaxUrl += "?pageIndex="
								+ ($scope.processingOrderPaging.pageIndex - 1);
						ajaxUrl += "&pageSize="
								+ $scope.processingOrderPaging.pageSize;
						ajaxUrl += "&sortedBy=id_desc";
