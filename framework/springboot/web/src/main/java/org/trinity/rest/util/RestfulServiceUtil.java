...
public class RestfulServiceUtil implements IRestfulServiceUtil {
    ...
    public RestfulServiceUtil(final IRestServer restServer, final IExceptionFactory exceptionFactory) {
        super();
    ...
    @Override
    public <T extends IResponse> T callRestService(final String token, final IHttpUrl url, final String subPath, final Object requestBody,
            final Object requestParamBean, final Class<T> responseType) throws IException {
        final T callRestService = callRestService(token, url.getFullPath(), subPath, url.getHttpMethod(), requestBody, requestParamBean,
                responseType);

        if (!callRestService.getErrors().isEmpty()) {
            final IException exception = exceptionFactory.createException("");

            callRestService.getErrors().forEach(item -> exception.addMessage(GeneralErrorMessage.UNKNOWN_EXCEPTION, item.getMessage()));
            throw exception;
        }
